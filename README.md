...
1. `internal-upload_v0.5.2_windows.zip`을 다운로드합니다.
...
서버와 클라이언트 릴리즈 버전은 웹 화면에 함께 표시됩니다. `v0.5.2`는 TCP 프로토콜 `v2`를 유지하므로 `v0.4.3` 이후 클라이언트와 프로토콜상 호환되지만, 실행 역할이 분리된 최신 클라이언트 ZIP 사용을 권장합니다. `v0.4.2` 및 이전 클라이언트는 서버 웹 화면에서 최신 ZIP을 다시 받아야 합니다.
...
현재 구조, 사용자별 평가, 안정성 근거, P0/P1/P2 목록과 단계별 검증 계획은 [v0.5.2 한국어 프로젝트 진단 보고서](https://github.com/sebia1993/-/blob/v0.5.2/docs/PROJECT_DIAGNOSTIC_AND_IMPROVEMENT_PLAN_KO.md)에 정리했습니다. 소스 저장소에서는 `docs/PROJECT_DIAGNOSTIC_AND_IMPROVEMENT_PLAN_KO.md` 경로로도 확인할 수 있습니다.
...
현재 정식 Release는 `v0.5.2`이며 실행용 asset은 `internal-upload_v0.5.2_windows.zip`과 SHA256 파일입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
